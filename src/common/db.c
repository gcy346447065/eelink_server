...
static int _db_insertOBJ(const char *imeiName, int ObjectType, const char *CCID, const char *IMSI)
{
    char query[MAX_QUERY];
    snprintf(query, MAX_QUERY, "insert into object(imei, ObjectType, CCID, IMSI) values(\'%s\', %d, \'%s\', \'%s\')", imeiName, ObjectType, CCID, IMSI);
...
{
#ifdef WITH_DB
    return _db_insertOBJ(imeiName, ObjectType, CCID, IMSI);
#else
    return 0;
#endif
}
