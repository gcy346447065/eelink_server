...
void http_requset_post_cb(struct evhttp_request *req, void *arg)
{
    ...
    switch(req->response_code)
    {
        ...
        case HTTP_OK + 1:
        {
            ...
            session->pfn(req->response_code, body, session->obj);
            free(body);
...
//            event_base_loopexit(http_req_post->base, 0);
        ...
        case HTTP_BADREQUEST:
        	...
        	session->pfn(req->response_code, NULL, session->obj);
        	return;
...
void *http_get(HTTP_SESSION* session, const char *url)
{
	...
	struct evhttp_connection *cn = evhttp_connection_base_new(session->base,
            NULL,
